...
internal class DeliveryDriverJobInstance : PickupDropoffBasedJob
{
	public DeliveryDriverJobInstance(CPlayer a_Owner) : base(a_Owner, EJobID.DeliveryDriverJob, EAchievementID.DeliveryJob, EAchievementID.CompleteDeliveryJob, "Delivery Driver", EDrivingTestType.Truck, EVehicleType.DeliveryDriverJob)
	{
		AddLevel(0, 29f);
		AddLevel(20, 38.0f);
		AddLevel(40, 51f);
		...
		// LS
		AddPickupLocation(EScriptLocation.LS, new Vector3(231.3397, -3131.756, 5.790275));
	...
	{
		a_Player.DeliveryDriverJobXP += a_XPGained;
		Database.Functions.Jobs.SaveDeliveryDriverJobProgress(a_Player.ActiveCharacterDatabaseID, a_Player.DeliveryDriverJobXP);
		return a_Player.DeliveryDriverJobXP;
	}

	public override int GetXP()
	{
		return m_Owner.Instance().DeliveryDriverJobXP;
	}
}