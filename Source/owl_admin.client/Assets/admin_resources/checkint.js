...
function AddNote() {
    ...
    TriggerEvent("SaveInteriorNote", note);
    TriggerEvent("ReloadCheckIntData");
    $('#adminnotes').val('').blur();
...
function Close() {
    TriggerEvent("CloseCheckInt");
}

function SetAllData(propertyDetails) {
    cachedDetails = JSON.parse(propertyDetails);

    var date = new Date(cachedDetails.lastUsed * 1000);

    $('#username').text(cachedDetails.Username);
    $('#characterName').text(cachedDetails.CharacterName);
    $('#propPrice').text("$" + cachedDetails.BoughtFor);
    $('#locked').text(cachedDetails.IsLocked);
    $('#propName').text(cachedDetails.PropertyName);
    $('#intID').text(cachedDetails.InteriorID);
    ...
    $('#propertyState').text(cachedDetails.State);
    $('#rentedPrice').text("$" + cachedDetails.RentFor);
    $('#lastUsed').text(date.toString());
